  ...
  const handleSearch = async (e: React.FormEvent, reset = false) => {
    ...
    try {
      ...
      if (result.leads.length === 0) {
        setError("Nessun profilo rispetta i criteri rigidi (min 20 post, min 300 follower, città in bio e presenza Reels). Prova a variare i termini.");
      } else {
      ...
      setError("Errore nella sincronizzazione dati: " + (err.message || 'Riprova.'));
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="max-w-[1600px] mx-auto px-8 h-20 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-indigo-600 font-bold uppercase tracking-[0.2em] mt-1">High-Quality Talent Sync</p>
            </div>
          </div>
      ...
      <div className="flex-grow flex flex-col lg:flex-row max-w-[1600px] mx-auto w-full">
        <aside className="w-full lg:w-96 p-8 lg:border-r border-slate-200 bg-white lg:sticky lg:top-20 lg:h-[calc(100vh-80px)] overflow-y-auto">
          <div className="space-y-8">
            <section>
              <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Parametri Professionali</h3>
              <form className="space-y-5">
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase mb-2">Network</label>
                ...
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase mb-2">Ruolo</label>
                  <select
                    value={query.role}
                ...
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase mb-2">Industria</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase mb-2">Followers Minimi</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase mb-2">Città (Obbligatoria in Bio)</label>
                  <input
                    type="text"
                    value={query.city}
                ...
                <div className="pt-6 flex flex-col gap-4">
                  ...
                  >
                    {isLoading ? 'Sincronizzazione...' : 'Sincronizza Database'}
                  </button>
            ...
            <section className="bg-amber-50 p-5 rounded-2xl border border-amber-100">
               <div className="flex items-center space-x-2 mb-2">
                 ...
                 <h4 className="text-[10px] font-black text-amber-800 uppercase tracking-widest">Quality Check Attivo</h4>
               </div>
               <p className="text-[11px] text-amber-700 leading-relaxed font-medium">
                 Profili filtrati per: +20 post, +300 followers, presenza Reels, foto profilo e menzione città.
               </p>
            </section>
          </div>
        </aside>
        ...
        <main className="flex-grow p-8 lg:p-12 bg-[#F8FAFC]">
          {error && (
            <div className="max-w-4xl mb-10 bg-white border-l-4 border-amber-400 text-slate-700 px-6 py-5 rounded-2xl flex items-start shadow-xl shadow-slate-200/50">
              ...
              <div>
                <p className="text-base font-black">Sistema di Qualità</p>
          ...
          {!isLoading && leads.length === 0 && !error && (
            <div className="flex flex-col items-center justify-center py-48">
              ...
              <h3 className="text-xl font-black text-slate-800 uppercase tracking-widest">In attesa di sincronizzazione</h3>
              <p className="text-sm text-slate-400 mt-2 font-medium">Configura i parametri professionali per caricare il database.</p>
            </div>
          )}
          ...
          {leads.length > 0 && !isLoading && (
            <div className="mt-24 border-t border-slate-200 pt-16 flex flex-col items-center">
              ...
              >
                ...
                <span className="text-xs font-black uppercase tracking-[0.3em] text-slate-300 group-hover:text-slate-900 transition-colors">Carica Altri Risultati</span>
              </button>
            </div>
          )}
          ...
          {sources.length > 0 && (
             <div className="mt-20 opacity-10 hover:opacity-100 transition-opacity">
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Database Source Map</p>
